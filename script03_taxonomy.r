...
data_phylum_top10 = data_phylum %>% 
                    mutate(taxon_name = ifelse(taxon_name %in% c("unclassified", "cannot be assigned to a (non-viral) phylum"), "unclassified", taxon_name))  %>%  # 2つの系統名をunclassifiedに変更纏める
                    arrange(-percent) %>% arrange(sample) %>%   # 相対存在量とサンプル名でソート
                    mutate(taxon_name = ifelse(taxon_name %in% c(head(taxon_name,10), "unclassified"), taxon_name, "Others")) %>%  # Top10とunclassified以外の系統名をOthersに変更

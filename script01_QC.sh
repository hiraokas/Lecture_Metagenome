...
# 解析環境構築
conda create -n env_metagenome seqkit sra-tools trim-galore bowtie2 ncbi-acc-download prinseq-plus-plus fastqc flash2 -c bioconda
...
# paired-endリードのマージ
flash2 --allow-outies QC/DRR267104_QC_1.fastq QC/DRR267104_QC_2.fastq --output-directory QC/ --output-prefix DRR267104_merge -t 4
flash2 --allow-outies QC/DRR267106_QC_1.fastq QC/DRR267106_QC_2.fastq --output-directory QC/ --output-prefix DRR267106_merge -t 4
flash2 --allow-outies QC/DRR267108_QC_1.fastq QC/DRR267108_QC_2.fastq --output-directory QC/ --output-prefix DRR267108_merge -t 4
flash2 --allow-outies QC/DRR267110_QC_1.fastq QC/DRR267110_QC_2.fastq --output-directory QC/ --output-prefix DRR267110_merge -t 4

# リード数等の基礎統計の確認
seqkit stat data/*.sra_*.fastq         #生リード
seqkit stat QC/*_QC_*.fastq            #QC後
seqkit stat QC/*_.extendedFrags.fastq  #マージ後

# DRR267104についてのみ表示
seqkit stat data/DRR267104.sra_*.fastq              #生リード
seqkit stat QC/DRR267104_QC_*.fastq                 #QC後
seqkit stat QC/DRR267104_merge.extendedFrags.fastq  #マージ後

# FastQCによるレポート出力（QC前後）
fastqc data/*.sra_*.fastq             -o data/
fastqc QC/*_QC_*.fastq                -o QC/
fastqc QC/*_merge.extendedFrags.fastq -o QC/
